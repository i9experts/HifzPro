...
const NAV_ITEMS = [
  { href: "/dashboard/parent",            icon: "🏠", label: "Home",       labelUr: "ہوم" },
  { href: "/dashboard/parent/diary",      icon: "📖", label: "Diary",      labelUr: "ڈائری" },
  { href: "/dashboard/parent/attendance", icon: "📅", label: "Attendance", labelUr: "حاضری" },
  { href: "/dashboard/parent/progress",   icon: "📊", label: "Progress",   labelUr: "ترقی" },
  { href: "/dashboard/parent/contact",    icon: "📞", label: "Contact",    labelUr: "رابطہ" },
];
...
export default function ParentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Detect standalone (installed PWA) for safe-area adjustments
    setIsStandalone(
    ...
    <ParentContext.Provider value={{ student, children: childrenList, activeChild, setActiveChild: handleChildSwitch, loading, refresh: () => fetchData(activeChild) }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          paddingTop: isStandalone ? "calc(12px + env(safe-area-inset-top))" : "12px",
        }}>
        ...
        {/* Page content — bottom padding accounts for nav + safe area */}
        ...
        <div style={{
          ...
          borderTop: `1px solid ${colors.n200}`,
            ...
            return (
              <Link key={item.href} href={item.href} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "8px 4px 6px", textDecoration: "none", transition: "all 0.15s" }}>
                <div style={{
                  ...
                  borderRadius: 10,
                  background: isActive ? colors.green50 : "transparent",
                  transition: "all 0.2s",
                }}>
